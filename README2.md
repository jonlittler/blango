...
```bash
pip3 install drf-yasg   # yet another swagger generator
```
...
class CustomerViewSet(viewsets.ModelViewSet):
    ...
    @action(methods=["get"], detail=True, name="Orders for customer")
    def orders(self, request, pk=None):
      customer = self.get_object()
      order_serializer = OrderDetailSerializer(customer.order_set, many=True)
      return Response(order_serializer.data)
```

<pre><mark>@action(methods=["get"], detail=True, name="Orders for customer")</mark></pre>
