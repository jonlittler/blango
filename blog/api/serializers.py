from rest_framework import serializers
from blog.models import Post, Tag, Comment
from blango_auth.models import User   # overrode default user

# Images
from versatileimagefield.serializers import VersatileImageFieldSerializer

class TagField(serializers.SlugRelatedField):
  def to_internal_value(self, data):
    try:
      return self.get_queryset().get_or_create(value=data.lower())[0]
    except (TypeError, ValueError):
      self.fail(f"Tag value {data} is invalid")


class UserSerializer(serializers.ModelSerializer):
  class Meta:
    model = User
    fields = ["first_name", "last_name", "email"]


class CommentSerializer(serializers.ModelSerializer):
  id = serializers.IntegerField(required=False)
  creator = UserSerializer(read_only=True)

  class Meta:
    model = Comment
    fields = ["id", "creator", "content", "modified_at", "created_at"]
    readonly = ["modified_at", "created_at"]


class PostSerializer(serializers.ModelSerializer):
  # tags = serializers.SlugRelatedField(slug_field="value", many=True, queryset=Tag.objects.all())
  tags = TagField(slug_field="value", many=True, queryset=Tag.objects.all())
  author = serializers.HyperlinkedRelatedField(queryset=User.objects.all(), view_name="api_user_detail", lookup_field="email")

  hero_image = VersatileImageFieldSerializer(
    sizes=[
      ("full_size", "url"),
      ("thumbnail", "thumbnail__100x100"),],
    read_only=True,
  )

  class Meta:
    model = Post
    # fields = "__all__"
    exclude = ["ppoi"]
    readonly = ["modified_at", "created_at"]


# Subclass of post serializer includes comments
class PostDetailSerializer(PostSerializer):
    # comments = CommentSerializer(many=True, read_only=True)
    comments = CommentSerializer(many=True)

    hero_image = VersatileImageFieldSerializer(
        sizes=[
          ("full_size", "url"),
          ("thumbnail", "thumbnail__100x100"),],
        read_only=True,
      )

    def update(self, instance, validated_data):

      # Remove the comments from the validated_data
      comments = validated_data.pop("comments")
      print("comments", comments)

      # Call super update() method to update our Post object
      instance = super(PostDetailSerializer, self).update(instance, validated_data)

      # Iterate over the list of comments 
      # If it has an id we assume the Comment already exists.
      for comment_data in comments:
        if comment_data.get("id"):
          continue
        comment = Comment(**comment_data)

        # Set the creator to the current user of the request
        comment.creator = self.context["request"].user

        # Link content_object of the Comment to the current Post
        comment.content_object = instance
        comment.save()

      return instance

class TagSerializer(serializers.ModelSerializer):
  class Meta:
    model = Tag
    fields = "__all__"
